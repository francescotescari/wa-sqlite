...
const DEFAULT_SQL = `
CREATE TABLE IF NOT EXISTS t1(a INTEGER, b INTEGER, c VARCHAR(100));
...
window.addEventListener('DOMContentLoaded', async function() {
  ...
  const refreshWorker = async (event) => {
    button.disabled = true;

    // Restart the worker.
    worker?.terminate();
    worker = new Worker('./demo-worker-concurrent.js', { type: 'module' });
    await new Promise(resolve => {
    ...
    config.useMutex = useMutex.checked;
    config.dbName = Math.round(Math.random() * 100000000).toString(16);
  ...
  let worker;
  select.addEventListener('change', refreshWorker);
  concurrency.addEventListener('change', refreshWorker);
  useMutex.addEventListener('change', refreshWorker);
